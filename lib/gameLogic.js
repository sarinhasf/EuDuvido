 ...
 *
 * "Passar":
 *  quem nao consegue responder paga 1 vida e a vez segue pro proximo vivo.
 *  A carta NAO e descartada - a mesa continua no mesmo top 10.
 *
...
export const estadoInicial = {
  ...
  ultimoDuvido: null,
  ultimoPasse: null, // { jogadorId, nome, vidas, eliminado } - so pro aviso na tela
  vencedorId: null,
  rodada: 0,
};
...
export function reducer(estado, acao) {
  switch (acao.type) {
    ...
    case 'LIMPAR_PASSE':
      return { ...estado, ultimoPasse: null };
    ...
    /**
     * acao: { jogadorId }
     * O jogador desistiu da vez: paga 1 vida e passa a bola.
     * A carta continua na mesa (precisaNovoTema segue como esta) porque a
     * mesa ainda nao esgotou o top 10 - so essa pessoa nao soube responder.
     */
    case 'PASSAR': {
      const idx = estado.jogadores.findIndex((j) => j.id === acao.jogadorId);
      // id inexistente ou jogador ja eliminado: ignora em vez de corromper o estado
      if (idx === -1 || !estado.jogadores[idx].vivo) return estado;

      const jogadores = estado.jogadores.map((j, i) =>
        i === idx ? { ...j, vidas: j.vidas - 1, vivo: j.vidas - 1 > 0 } : j,
      );

      return aplicarFimDeJogo({
        ...estado,
        jogadores,
        vezIndex: proximoVivo(jogadores, idx),
        ultimoPasse: {
          jogadorId: acao.jogadorId,
          nome: estado.jogadores[idx].nome,
          vidas: jogadores[idx].vidas,
          eliminado: !jogadores[idx].vivo,
        },
      });
    }
